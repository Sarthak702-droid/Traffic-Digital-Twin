...
export function pendingCommand():string|null {try{return sessionStorage.getItem("twin-uncertain-command")}catch{return null}}
export function clearPendingCommand(){try{sessionStorage.removeItem("twin-uncertain-command")}catch{};window.dispatchEvent(new Event("command-outcome"));}
export async function request<T>(path:string,options?:RequestInit):Promise<T>{
 ...
 if(commandId)headers.set("Idempotency-Key",commandId);
 ...
 try{
  ...
  if(!response.ok){const error=data as {message?:string;command_id?:string;outcome?:string};throw new ApiError(error.message||`Request failed (${response.status})`,response.status,error.command_id||commandId,mutation&&(response.status>=500||error.outcome==="unknown"))}
  try{return validateResponse(path,data) as T}catch{throw new ApiError("Service response does not match the expected contract. Data is unavailable.",502,commandId,mutation)}
 }catch(error){
  ...
  if(e.uncertain&&e.commandId){try{sessionStorage.setItem("twin-uncertain-command",e.commandId)}catch{};window.dispatchEvent(new Event("command-outcome"))}
