...
export function SessionPanel({onReviewFinished}:{onReviewFinished?:(commandId:string)=>void}={}){
 const session=useSession();const client=useQueryClient();const [uncertain,setUncertain]=useState<string|null>(null);const [reviewed,setReviewed]=useState(false);
 ...
 {session.isSuccess&&!session.isError ? <p>Local demonstration session · <strong>{session.data.actor}</strong> · {session.data.role}</p> : session.isError ? <p role="alert"><strong>Local demonstration session unavailable.</strong> Operator controls are paused until the Go API reconnects. No username or password is required.</p> : <p role="status">Connecting to the local demonstration operator…</p>}
 ...
 <button disabled={!reviewed||!(outcome.data?.status==="completed" || (outcome.error as {status?:number}|undefined)?.status===404)} onClick={()=>{const commandId=uncertain;clearPendingCommand();setReviewed(false);onReviewFinished?.(commandId);client.invalidateQueries()}}>Finish review (does not retry)</button>
 </div>}
 </section>;
}
