...
- Deterministic aggregate-flow setup/reset, 1 Hz state, conservation forecasts at 30/60/120/300 seconds, aggregate comparison and bounded candidates. Go and Python include safety validators; signal scheduler defers timing changes to clearance boundaries. Three seeded scenarios and prerecorded replay assets exist.
...
**Required fix:** Mount a controlled, visible fallback/restart flow using the normal gateway path; distinguish replay, completion and live state throughout. Return explicit errors for missing dependencies and test aggregate-runtime/intelligence loss, missing replay files and database loss.
...
- Python shared, safety, service and intelligence suites: historical sandbox evidence predates the aggregate migration. Current verification uses the aggregate simulation, intelligence, and vision suites without an external simulator.
